...
-- 테이블 열 추가입력
ALTER TABLE book ADD price NUMBER NOT NULL;
...
SELECT bookname 도서이름, price 도서가격
FROM book;
...
-- 가격이 10000원 이상 20000원 이하인 도서를 검색하시오 (AND)
SELECT *
FROM book
WHERE price >= 10000
AND price <= 20000;
...
-- '축구의 역사'를 출간한 출판사를 검색하시오 (LIKE 연산자)
SELECT publisher
FROM book
...
-- 축구에 관한 도서 중 가격이 20000 이상인 도서를 찾으시오
SELECT *
FROM book
WHERE bookname LIKE '%축구%'
AND price >= 20000;
